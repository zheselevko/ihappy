...
class ModelCatalogProductall extends Model {
	...
	public function getTotalProducts($data = array()) {
		...
		if (!empty($data['filter_tag'])) {
			...
			if(isset($_GET['model']) && (int)$_GET['model']>0){
					$sql .="INNER JOIN " . DB_PREFIX . "product_option_value AS options ON p.product_id = options.product_id";

	}		
		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
										if(isset($_GET['model']) && (int)$_GET['model']>0){
					$sql .="AND options.option_value_id =".(int)$_GET['model'];
	...
	public function getProduct($product_id) {
		...
		public function getProduct_cat($product_id) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$query = $this->db->query("
		SELECT `oc_category_description`.`name`
FROM `oc_product`
LEFT JOIN `oc_product_to_category` ON `oc_product`.`product_id` = `oc_product_to_category`.`product_id`
INNER JOIN `oc_category_description` ON `oc_category_description`.`category_id` = `oc_product_to_category`.`category_id`
INNER JOIN `oc_category` ON `oc_category`.`category_id` = `oc_product_to_category`.`category_id`
WHERE `oc_category_description`.`language_id` =".(int)$this->config->get('config_language_id')."
AND `oc_product`.`product_id` =".(int)$product_id."
